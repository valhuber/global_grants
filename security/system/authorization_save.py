"""
System support for role based authorization.  Checkpoint with multiple strategies
...
tenant_early_binding = False
''' experiment - create "normal grants" in ctor, rather than exec_grants '''
...
class GlobalsByClassName():

    globals_by_class_name = Dict[str, list['GlobalTenantFilter']]
    """ key by class_name, return list['GlobalTenantFilter'] """

    def init(global_tenant_filter: 'GlobalTenantFilter'):  
        if global_tenant_filter.class_name not in GlobalsByClassName.globals_by_class_name:
            GlobalsByClassName.globals_by_class_name[
                global_tenant_filter.class_name] = []  # list[(attrname, 'GlobalTenantFilter')]
        filter_list = GlobalsByClassName.globals_by_class_name[global_tenant_filter.class_name]
        filter_list.add(global_tenant_filter)
...
class GlobalTenantFilter():
    ...
    grants_by_multi_tenant_attribute_name : Dict[ str, list['GlobalTenantFilter'] ] = {}
    '''
        Dictionary of filters applied to each entity with a tenant_id
        TODO: key is name of multi-tenant attr?, obj is a GlobalTenantFilter (a SQLAlchemy filter lambda?)
    '''
                ...
                if (f"'{models_name}." in str(each_class_def_str) and
                    ...
                    for each_column in columns:
                        if each_column.name == multi_tenant_attribute_name:
                            ...
                            if table_name.startswith("Customer"):
                                if tenant_early_binding:
                                    use_lambda = False
                                    if use_lambda:
                                        pass
                                        ...
                                        Grant(  on_entity = models.Customer,
                                                to_role = "manager",
                                                filter_debug=self.lambda_str,
                                                filter = self.lambda_filter)  # eval(self.lambda_str))
                                else:  # fails: 'str' object has no attribute '__subclasses__'
                                    if self.class_name not in GlobalTenantFilter.globals_by_class_name:
                                        GlobalTenantFilter.globals_by_class_name[self.class_name] = []
                                    filter_list = GlobalTenantFilter.globals_by_class_name[self.class_name]
                                    filter_list.append(self)
                                break  # next resource class


    def getLambdaFunction(self, entity_class) -> object:
        """ return filter lambda from saved string

        TODO serious hack - lambda: models.Customer.Client_id == 1... needs to become bin exp from str

        TODO: maybe rename to get_filter_as_lambda

        Args:
            entity_class (_type_): _description_

        Returns:
            object: TODO - is a class, or a string that gets turned into 1
        """
        lambda_fn = self.filter.replace("{entity_class}",f"models.{entity_class}")
        #return_class =  lambda: models.Customer.Client_id == 1  # serious hack
        return_class = exec(lambda_fn)
        return return_class()
...
class Grant:
    ...
    def exec_grants(entity_name: str, crud_state: str, orm_execute_state: any = None, property_list: any = None) -> None:
        ...
        #########################################
        # Apply GlobalTenantFilter 
        #########################################
        if crud_state == "is_select" and property_list:
            if entity_name in GlobalTenantFilter.globals_by_class_name:
                global_grants_for_entity = GlobalTenantFilter.globals_by_class_name[entity_name]
                for each_global_grant in global_grants_for_entity:
                    grant_list.append(each_global_grant.lambda_filter())
                    security_logger.debug(f"added Global tenant filter for {entity_name}: {each_global_grant.lambda_str}")
